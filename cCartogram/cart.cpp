...
double** cart_dmalloc(int xsize, int ysize)
{
  ...
  userrho = (double **) malloc(xsize*sizeof(double*));
  *userrho = (double *) fftw_malloc(xsize*ysize*sizeof(double));
...
void cart_makews(int xsize, int ysize)
{
  ...
  for (s=0; s<5; s++) rhot[s] = (double *) fftw_malloc(xsize*ysize*sizeof(double));
  fftrho = (double *) fftw_malloc(xsize*ysize*sizeof(double));
  fftexpt = (double *) fftw_malloc(xsize*ysize*sizeof(double));

  for (s=0; s<5; s++) {
    vxt[s] = (double **) malloc((xsize+1)*sizeof(double*));
    for (i=0; i<=xsize; i++) vxt[s][i] = (double *) malloc((ysize+1)*sizeof(double));
  }
  for (s=0; s<5; s++) {
    vyt[s] = (double **) malloc((xsize+1)*sizeof(double*));
    for (i=0; i<=xsize; i++) vyt[s][i] = (double *) malloc((ysize+1)*sizeof(double));
  }

  expky = (double *) malloc(ysize*sizeof(double));
